...
**Latency:**
In our scaling experiments, we keep the buffer size per GPU constant while increasing the node count. As a result--in tree-- are  In strong scaling with thousands of GPUs, the work per GPU becomes so small (<MB) that the network latency becomes significant. It is future research to find latency-oriented hierarchical compositions for improving scalability.
